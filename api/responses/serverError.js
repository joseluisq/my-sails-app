...
module.exports = function serverError(data, options) {
    ...
    // Custom response
    var HTTPStatus = HTTPStatusService;
    ...
    // Set status code
    res.status(HTTPStatus.ERROR.code);

    // Log error to console
    if (data !== undefined) {
        sails.log.error('Sending ' + HTTPStatus.ERROR.code.toString() + ' ("Server Error") response: \n', data);
    }
    else
    ...
    if (req.wantsJSON) {
        return res.jsonx({code: HTTPStatus.ERROR.code, data: data});
    }
    ...
    // but fall back to sending JSON(P) if any errors occur.
        ...
        return res.view(HTTPStatus.ERROR.code.toString(), {data: data}, function (err, html) {
